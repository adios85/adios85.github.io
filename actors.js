...
var pluginTranslations = {  
    ...
    persons_not_found: {  
        ...
        en: "No persons found",          uk: "Особи не знайдені",  
    ...
    this.list = function(params, onComplete, onError) {  
        ...
        if (pageIds.length === 0) {  
            onComplete({  
                results: [],  
                page: page,  
                total_pages: Math.ceil(personIds.length / PAGE_SIZE),                  total_results: personIds.length  
            });  
            return;  
        }  
                ...
                new Lampa.Reguest().silent(url, function(response) {  
                    try {  
                        ...
                        if (json && json.id && json.name) {  
                            ...
                            error("[PERSON-PLUGIN] Invalid person data:", json);                        }
                    } catch (e) {  
...
function startPlugin() {  
    ...
    var personsService = new PersonsService();      Lampa.Api.sources[PLUGIN_NAME] = personsService;  
    ...
    menuItem.on("hover:enter", function() {  
        Lampa.Activity.push({  
            ...
            error("[PERSON-PLUGIN] Error opening persons list:", err);
    ...
    function waitForContainer(callback) {  
        ...
        if (initialCheck) {  
            log("[PERSON-PLUGIN] Container found immediately");              callback();  
        } else {  
    ...
    function checkCurrentActivity() {  
        ...
        if (activity && activity.component === 'actor') {  
            ...
            if (activity.id && activity.media_type === 'person') {  
                currentPersonId = parseInt(activity.id, 10);  
            }   
            else if (activity.params && activity.params.id && activity.params.media_type === 'person') {  
            ...
            if (currentPersonId) {  
                ...
                error("[PERSON-PLUGIN] No valid ID found in current activity");  
            }  
        }  
    }  
    ...
    Lampa.Listener.follow('activity', function(e) {  
        ...
        if (e.type === 'start' && e.component === 'actor') {  
            ...
            if (e.object && e.object.id && e.object.media_type === 'person') {  
                ...
                error("[PERSON-PLUGIN] Invalid actor object:", e.object);
            }  
        }  
    ...
    Lampa.Listener.follow('storage', function(e) {
        if (e.name === PERSONS_KEY) {
            log("[PERSON-PLUGIN] Persons list changed, clearing cache");
            clearCache();
        }
    });
    ...
    window.personsPluginDebug = {
        ...
        getSubscribed: getPersonIds,
        getCurrent: function() { return currentPersonId; },
