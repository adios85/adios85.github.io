!function() {
    ...
    var my_logging = true; // Включить логирование (для отладки)
    ...
    function PersonsService() {  
                ...
                (function(i) {  
                    ...
                    new Lampa.Reguest().silent(url, function(response) {  
                        try {  
                            ...
                            if (json && json.id) {  
                                // Создаём объект персоны с явным onclick  
                                var personCard = {  
                                    id: json.id,  
                                    title: json.name,  
                                    name: json.name,  
                                    ...
                                    media_type: "person",  
                                    source: "tmdb",  
                                    component: "actor", // на всякий случай  
                                    known_for_department: json.known_for_department,  
                                    gender: json.gender || 0,  
                                    popularity: json.popularity || 0,  
                                    // Явный обработчик клика  
                                    onclick: function() {  
                                        log("[PERSON-PLUGIN] Click on person:", json.id, json.name);  
                                        Lampa.Activity.push({  
                                            component: "actor",  
                                            id: json.id,  
                                            title: json.name,  
        ...
        // Метод get (для детальной информации, может вызываться при открытии)  
    ...
    function startPlugin() {  
        ...
        // Функция ожидания контейнера на странице актёра  
        ...
        // Проверка текущей активности при старте  
