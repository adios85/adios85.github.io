!function() {
    ...
    // Класс для сервиса персон  
    function PersonsService() {  
        var self = this;  
        var cache = {};  

        // Метод для получения списка подписок (пагинация)  
                ...
                (function(i) {  
                    ...
                    new Lampa.Reguest().silent(url, function(response) {  
                        try {  
                            ...
                            if (json && json.id) {  
                                // Формируем карточку персоны с явным указанием компонента и типа  
                                var personCard = {  
                                    id: json.id,  
                                    title: json.name,  
                                    name: json.name,  
                                    ...
                                    type: "person",               // или "actor" – рекомендуется "person"  
                                    media_type: "person",  
                                    source: "tmdb",  
                                    component: "actor",           // явно указываем, что открывать компонент actor  
        ...
        // Метод для получения детальной информации о конкретной персоне (вызывается при клике)  
    ...
    function startPlugin() {  
        ...
        // Функция для ожидания появления контейнера на странице актёра  
