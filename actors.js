        ...
        for (var i = 0; i < pageIds.length; i++) {  
            (function(i) {  
                ...
                new Lampa.Reguest().silent(url, function(response) {  
                    try {  
                        ...
                        if (json && json.id) {  
                            var personCard = {  
                                ...
                                // Эти поля критически важны для нашего перехватчика ниже
                                type: "person",  
                                source: "tmdb",  
                    ...
                    } catch (e) { error("[PERSON-PLUGIN] Error parsing", e); }  
                    checkComplete();  
                }, function(errorMsg) { checkComplete(); });  
            })(i);  
        }  
...
function startPlugin() {  
    // --- МАГИЯ ПЕРЕХВАТА МАРШРУТИЗАЦИИ LAMPA ---
    // Перехватываем метод push, чтобы Lampa не открывала актеров как фильмы
    if (!window.personPluginInterceptorAdded && Lampa.Activity && Lampa.Activity.push) {
        var originalActivityPush = Lampa.Activity.push;
        Lampa.Activity.push = function(object) {
            // Если Lampa пытается открыть компонент 'full' (фильм), но данные принадлежат персоне
            if (object && object.component === 'full' && object.movie && 
               (object.movie.type === 'person' || object.movie.original_type === 'person' || object.movie.media_type === 'person')) {
                
                log("[PERSON-PLUGIN] Перехват маршрута: перенаправляем фильм на актера", object.movie.id);
                
                // Принудительно меняем маршрут на компонент актера
                object.component = 'actor';
                object.id = object.movie.id;
                object.url = 'person/' + object.movie.id;
                object.title = object.movie.name || object.movie.title;
            }
            return originalActivityPush.apply(this, arguments);
        };
        window.personPluginInterceptorAdded = true; // Защита от двойного перехвата
    }
    // -------------------------------------------
