...
function initPlugin() {
    try {
        ...
        function addButtonToContainer(bottomBlock) {
            ...
            btn.classList.add(isSubscribed ? 'button--unsubscribe' : 'button--subscribe');            btn.setAttribute('data-focusable', 'true');
            btn.innerHTML = `
        ...
        // --- Сервис персон ---
        function PersonsService() {            this.list = function(params, onComplete, onError) {
                ...
                function checkComplete() {
                    ...
                    if (loaded >= pageIds.length) {
                        var valid = results.filter(function(item) {
                            return item && item.id;
                        });
        ...
        Lampa.Lang.add({
            ...
            persons_plugin_unsubscribe: pluginTranslations.unsubscribe,            persons_plugin_not_found: pluginTranslations.persons_not_found,
        ...
        Lampa.Listener.follow('storage', function(e) {            if (e.name === PERSONS_KEY) {
                clearCache();
