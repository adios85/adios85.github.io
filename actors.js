    ...
    function startPlugin() {
        ...
        function checkCurrentActivity() {
            ...
            if (activity && activity.component === 'person') {
                log("[PERSON-PLUGIN] Current activity is person page");
                ...
                // Если ID не найден в стандартных местах, пробуем из URL
                    ...
                    var match = location.pathname.match(/\/view\/person\/(\d+)/);
                    if (match && match[1]) {
                        currentPersonId = parseInt(match[1], 10);
                        log("[PERSON-PLUGIN] Got person ID from URL:", currentPersonId);
                    }
                }
        ...
        Lampa.Listener.follow('activity', function(e) {
            ...
            // Для страницы актера
            if (e.type === 'start' && e.component === 'person') {
                log("[PERSON-PLUGIN] person page started");
                
                if (e.object && e.object.id) {
                    currentPersonId = parseInt(e.object.id, 10);
                    log("[PERSON-PLUGIN] Found person ID in e.object.id:", currentPersonId);
