...
function addButtonToContainer(bottomBlock) {  
    ...
    var buttonText = Lampa.Lang.translate(isSubscribed? 'persons_plugin_unsubscribe' : 'persons_plugin_subscribe');  
    ...
    button.on('hover:enter', function() {  
        ...
        button.toggleClass('button--subscribe button--unsubscribe')  
             .find('span').text(Lampa.Lang.translate(added? 'persons_plugin_unsubscribe' : 'persons_plugin_subscribe'));  
    });  
                ...
                new Lampa.Reguest().silent(url, function(json) {  
                    if (json && json.id) {  
                        var card = {  
                            ...
                            // КЛЮЧЕВОЕ ИСПРАВЛЕНИЕ ДЛЯ МАРШРУТИЗАЦИИ:
                            type: "actor",
                            method: "actor",
                            component: "actor", // Явно указываем компонент для открытия профиля
                            source: "tmdb"
                        };  
                        cache[id] = card;  
                        results.push(card);  
...
function startPlugin() {  
    ...
    // Слушатель событий для добавления кнопки на странице актера
    Lampa.Listener.follow('activity', function(e) {  
        if (e.type === 'start' && e.component === 'actor') {  
            currentPersonId = parseInt(e.object.id, 10);  
            var wait = setInterval(function() {  
                var container = $('.person-start__bottom');  
                if (container.length) { clearInterval(wait); addButtonToContainer(container); }  
            }, 200);  
            setTimeout(function() { clearInterval(wait); }, 5000);  
        }  
    });

    // ПРИНУДИТЕЛЬНЫЙ ПЕРЕХВАТ КЛИКОВ (для защиты от неверной маршрутизации Lampa) [1]
    Lampa.Listener.follow('app', function (e) {
        if (e.type == 'click' && e.item && (e.item.type == 'actor' |

| e.item.method == 'actor')) {
            e.item.component = 'actor'; // Гарантируем открытие профиля
        }
    });
}  
