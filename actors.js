!function() {
    ...
    // Сервис персон — УЛЬТРА-ЧИСТАЯ ВЕРСИЯ
    function PersonsService() {
                ...
                (function(i) {
                    ...
                    new Lampa.Reguest().silent(url, function(response) {
                        try {
                            ...
                            if (json && json.id) {
                                // Максимально упрощённый объект — только самое необходимое
                                var personCard = {
                                    id: json.id,
                                    title: json.name,
                                    ...
                                    type: "person",
                                    media_type: "person"
                                    // Никаких source, component, action, url и т.д.
                                };

                                // Добавляем свойство onclick, которое нельзя перезаписать
                                Object.defineProperty(personCard, 'onclick', {
                                    configurable: false,
                                    enumerable: true,
                                    value: function() {
                                        log("[PERSON-PLUGIN] >>>>>>>>>>>>>>>>>>>>>>>>>> FORCED CLICK <<<<<<<<<<<<<<<<<<<<<<<<<<", json.id, json.name);
                                        
                                        // Немедленно пытаемся открыть страницу актёра
                                        Lampa.Activity.push({
                                            component: "actor",
                                            id: json.id,
                                            title: json.name,
                                        ...
                                        // Через 50 мс повторяем, чтобы перебить возможное стандартное действие
                                        setTimeout(function() {
                                            log("[PERSON-PLUGIN] Forcing navigation again (delayed)");
        ...
        // Метод get — минимально необходимый, возвращает те же данные
